...
const teams = {
  ...
  N1: {
    headers: ["Colaborador", "Registros Operacional", "Registro Financeiro", "O.S Aberta a Campo", "Ligação Atendida", "Ligação Recusada", "Atendimento OPASuite", "Avaliação Individual", "Tempo Médio de Atendimento", "Tempo Médio de Resposta"],
    goals: {
      ...
      "Ligação Atendida": { target: 38, direction: "up" },
      "Ligação Recusada": { target: 15, direction: "down" },
...
function detectBlockWeek(rows, blockIndex, fallbackIndex) {
  for (let i = blockIndex; i >= Math.max(0, blockIndex - 8); i -= 1) {
    const text = normalize(rows[i].join(" "));
    if (text.includes("1 SEMANA")) return "s1";
    if (text.includes("2 SEMANA")) return "s2";
    if (text.includes("3 SEMANA") && fallbackIndex <= 2) return "s3";
    if (text.includes("3 SEMANA") && fallbackIndex >= 3) return "s4";
    if (text.includes("4 SEMANA")) return "s4";
  }
...
function parseBlock(rows, startIndex, team) {
  ...
  for (let i = startIndex + 1; i < rows.length; i += 1) {
    ...
    if (team === "N2") {
      ...
      parsed.push([
        name,
        normalizeImportedValue(row[1]),
        normalizeImportedValue(row[2]),
        normalizeImportedValue(row[3]),
        normalizeImportedValue(row[4]),
        normalizeImportedValue(row[5]),
        normalizeImportedValue(row[6]),
        normalizeImportedValue(row[7]),
        normalizeImportedValue(row[8], "time"),
        normalizeImportedValue(row[9], "time")
      ]);
    }
  }
...
function chartOptions() {
  return {
    ...
    plugins: {
      ...
      tooltip: { backgroundColor: "#0c1118", borderColor: "#2b3947", borderWidth: 1, titleColor: "#edf4fb", bodyColor: "#dfe8f2" }
    },
    scales: {
      x: { ticks: { color: "#91a0b2", maxRotation: 35 }, grid: { color: "rgba(43, 57, 71, 0.45)" } },
